function [net_process_lca, stage_breakdwn,stage_breakdwn_prc,consum_breakdwn, consum_breakdwn_prc, aware_outputs, traci_vec, cf_vec, nerc_out, bC_balance] = LCA_HTL_HM_v3(co2_capt_lci, cult_out, storage_fcn_out, HTL_process_mod_out, coord, direct_cap, file_id, aware_cult, transp_vec, annual_uptime, grid_red, biogenic_c_in_gas_phase)

load background_data.mat aware_data 

% LCA module, take process outputs and load data for process consumables
cult_mean =  str2double(cult_out(:,2));
storage_mean =  str2double(storage_fcn_out(:,2));
HTL_process_mod_out =  str2double(HTL_process_mod_out(:,2));
dewatering_energy = sum(cult_mean([11,14:16],:)); % cult_out(14:16) to include UV energy, cult_out(14:15) to exclude UV energy.
bC_HTL_gas = biogenic_c_in_gas_phase*(12/44); % kg CO2 converted to kg C
...
blue_wd_monthly = aware_cult(1,:); 
biomass_monthly = aware_cult(2,:);
...
bC_uptake = (-cult_mean(26))*annual_uptime*1000*(12/44); % kg C/yr - biogenic C uptake in biomass growth determined in cultivation model as tonnes CO2/day
bC_storage_loss = storage_mean(11)*(0.54); % kg C/yr - storage losses are subtracted from uptake credit and equals kg AFDW lost/yr * 54% carbon = kg C/yr
bC_HTL_CO2_vent = bC_HTL_gas; % kg C in CO2 emitted from HTL vent per year - calculation above (line 10)
bC_HTL_aqueous = cult_mean(26)*annual_uptime*1000*HTL_process_mod_out(25)*(12/44); %kg C/yr in aqueous phase (losses) - tonnes CO2/day*days*1000kg/ton*12/44*% C in that ends up in aqueous
bC_fuel_comb = end_use(1,2)*fuel_output*(12/44); % kg C/yr in fuel combustion emissions - all fuels - CO2 in combustion gas times molar ratio = kg C/yr in end use emissions

net_bC = bC_uptake + bC_storage_loss + bC_HTL_CO2_vent + bC_HTL_aqueous + bC_fuel_comb;

    % Adjust carbon balance by adjusting C in char and aqueous
    if net_bC <= 0 % if net_bC is negative it means total uptake > total release - or there is unreleased C that needs to be accounted for
        if HTL_process_mod_out(20) == 0 % if no char yield from AFDW biomass (no HTL solids just ash)
            bC_HTL_solids = 0; % carbon in HTL solids is zero and bC release in aqueous is under-estimated
            bC_HTL_aqueous = bC_HTL_aqueous - net_bC; % adjust C in aqueous phase 
        else % if there is biochar
            bC_HTL_solids = -1*net_bC; %bC in HTL solids = balance of bC carbon --> solved for by difference as end-use (biocrude), HTL gas, and HTL aqueous are accounte for
        end 
    else % if net_bC is > 0 than it means there is more relase than uptake
        if HTL_process_mod_out(20) == 0 % if no char yield from AFDW biomass (no HTL solids just ash)
            bC_HTL_solids = 0; % carbon in HTL solids is zero and bC release in aqueous is under-estimated
            bC_HTL_aqueous = bC_HTL_aqueous - net_bC; % adjust C in aqueous phase 
        else % if there is biochar set carbon in char to 3% based on Leow et al. and adjust aqueous
            bC_HTL_solids = cult_mean(26)*annual_uptime*1000*(12/44)*0.03; %bC in HTL solids = balance of bC carbon --> solved for by difference as end-use (biocrude), HTL gas, and HTL aqueous are accounte for
            net_bC = bC_uptake + bC_storage_loss + bC_HTL_CO2_vent + bC_HTL_aqueous + bC_fuel_comb + bC_HTL_solids;
            bC_HTL_aqueous = bC_HTL_aqueous - net_bC; % adjust C in aqueous phase 
        end 
    end 

% Biogenic carbon distribution - used to check - not used in LCA function
bC_balance = [bC_uptake bC_storage_loss bC_HTL_CO2_vent bC_HTL_aqueous bC_fuel_comb bC_HTL_solids]; 

% [1. Carbon capture, 2. Cultivation, 3. Dewatering, 4. Storage, 5. Conversion, 6. Transp, 7. Combustion, 8. Credits]
LCI_table = [0,0, 0, 0, 0, 0, 0,bC_uptake*(44/12); % portion of CO2 uptake balanced by release of biogenic carbon, kg per yr
             ...
             0,0,0,0,bC_HTL_aqueous*(44/12),0,0,0;% kg CO2/yr from aqueous phase to close bC mass balance
             0,0,0,0,0,0,0,0];% biogenic CO2 stored in HTL solids kg CO2/yr was --> as of 1/24/25 assuming carbon in solids is released through decomposition in soils
...
cf_vec = aware_data(aware_data(:,1) == file_id, 2:end);
% Cultivation
