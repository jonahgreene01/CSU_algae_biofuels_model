...
if ~hold_state
	...
	righttext = text(lxc, lyc, [repmat('  ', length(labels), 1) labels]);
    set(righttext, 'Rotation', 0)
    
	% Plot bottom labels (no b - only a c)
    [lxb, lyb] = terncoords(majorticks, zerocomp, 1-majorticks); % fB = 1-fA
	bottomtext = text(lxb - 0.05, lyb - 0.05, labels, 'VerticalAlignment', 'Top');
    set(bottomtext, 'Rotation', 60)
	
	% Plot left labels (no a, only c b)
	[lxa, lya] = terncoords(zerocomp, 1-majorticks, majorticks);
	lefttext = text(lxa-0.03, lya+0.06, labels);
    set(lefttext, 'Rotation', -60)
